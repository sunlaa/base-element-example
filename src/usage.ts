import { BaseElement, ParamsOmitTag } from './script.js';
const someContent =
...
class Button extends BaseElement {
  constructor(text: string, className: string[], callback: (e: Event) => void) {
...
function createSection(params: ParamsOmitTag) {
  return new BaseElement({ tag: 'section', ...params });
}
...
const parent = new BaseElement(
  { classes: ['parent'] },
  new BaseElement({ classes: ['child-1'] }),
  new BaseElement(
    { classes: ['child-2'] },
    new BaseElement({ classes: ['child-child'] })
  ),
  new BaseElement({ classes: ['child-3'] })
);
